...
def bound_terms(data_batch, variational_posterior_fn, decoder_fn):
    ...
    return likelihood_term, kl_term, z, variational_posterior, decoder_likelihood
...
if __name__ == '__main__':
    ...
    likelihood_term, kl_term, z, variational_posterior, decoder_likelihood = \
        bound_terms(data_var, encoder, standard_decoder)

    # train_elbo = likelihood_term - kl_term

    ##################  constraint
    with tf.variable_scope("constraint", reuse=tf.AUTO_REUSE):
        alpha = -70
        lagrangian_var = tf.get_variable("lagrangian_var", shape=[1], dtype=tf.float32)
        lagrangian_multiplier = tf.nn.softplus(lagrangian_var)

    loss = kl_term + lagrangian_multiplier * (alpha - likelihood_term)
    ...
    vae_update_op = optimizer.minimize(loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="encoder") +
                                                  tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="decoder"))
    with tf.control_dependencies([vae_update_op]):
        update_op = optimizer.minimize(-loss, var_list=[lagrangian_var])
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 100 == 0:
                iteration_loss, iteration_kl, iteration_likelihood, lag_val = sess.run(
                    [loss, kl_term, likelihood_term, lagrangian_multiplier])
